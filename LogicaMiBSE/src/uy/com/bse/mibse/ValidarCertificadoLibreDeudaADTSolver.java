...
public class ValidarCertificadoLibreDeudaADTSolver extends AbstractSolver {

	private static final Logger LOG = LogManager.getLogger(ValidarCertificadoLibreDeudaADTSolver.class);
		...
		try {
			...
			if (!rvalidado.getHayError().booleanValue() && rvalidado.getUser() != null) {
				res = checkNull(new ServiciosMiBsePersist()
						.validarCertificadoLibreDeudaADT((ParamValidarCertificadoLibreDeudaADT) param));
				if (!res.getHayError().booleanValue()) {
					resultado = (ResultValidarCertificadoLibreDeudaADT) res;
				} else {
