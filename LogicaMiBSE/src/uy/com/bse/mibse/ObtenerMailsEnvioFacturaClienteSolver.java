...
public class ObtenerMailsEnvioFacturaClienteSolver extends AbstractSolver {

	private static final Logger LOG = LogManager.getLogger(ObtenerMailsEnvioFacturaClienteSolver.class);
		...
		try {
			...
			if (!rvalidado.getHayError().booleanValue() && rvalidado.getUser() != null) {
				res = checkNull(new ServiciosMiBsePersist()
						.obtenerMailsEnvioFactura((ParamObtenerMailsEnvioFacturaCliente) param));
				if (!res.getHayError().booleanValue()) {
					resultado = (ResultObtenerMailsEnvioFacturaCliente) res;
				} else {
