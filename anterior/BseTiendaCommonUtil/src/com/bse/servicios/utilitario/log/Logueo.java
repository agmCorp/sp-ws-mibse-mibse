...
public class Logueo {
     ...
     * @param pad
     */
    public void setParametro(final String clave, final String valor, int pad) {
        String claveFormateada = clave;
        if ( clave!=null && clave.length()<pad ) { 
                claveFormateada = String.format("%-"+Integer.toString(pad)+"s", clave); 
        }
        if (valor == null) {
            this.parametros.add(claveFormateada + ": [null]");
        } else {
            this.parametros.add(claveFormateada + ": [" + valor + "]");
        }
    }
